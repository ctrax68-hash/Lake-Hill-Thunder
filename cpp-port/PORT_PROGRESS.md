...
### Step 2: glTF skinned-mesh import pipeline

  Adds the CPU-side glTF/GLB parsing and the GPU-side skinned-mesh upload/
  draw path the combined tire-model/Unreal-animation plan's Step 2 called
  for -- a skinned rig's geometry, joint hierarchy, and inverse bind
  matrices flow from a glTF asset into a real bgfx-drawable mesh, ready for
  Step 3 to drive its joints from live physics state (wheel spin, suspension
  travel) instead of baked animation clips. Entirely additive and dormant:
  nothing in the actual game (main.cpp's render loop, the flat car quad)
  calls any of this yet, so the tire-model/drivetrain/suspension regression
  numbers from Steps 1/1b/1c are untouched by this pass.

  **`src/render/mesh_import.h`/`.cpp`** (bgfx-free, ctest-covered): parses a
  glTF/GLB buffer via `cgltf` (MIT, single-header) into a plain
  `ImportedMesh` -- vertices (position/normal/uv/joint-indices/weights),
  indices, an optional skin (`ImportedJoint` per joint: name, parent index,
  local bind matrix via `cgltf_node_transform_local()`, inverse bind matrix),
  and material base color (factor + optional embedded texture bytes). Two
  deliberate deviations from the original plan text, both logged rather than
  silently assumed:
  - **cgltf reused, not re-vendored**: already present at
    `third_party/bgfx.cmake/bgfx/3rdparty/cgltf/cgltf.h` (bgfx's own
    `geometryc` tool depends on it, though that tool itself stays disabled
    via `BGFX_BUILD_TOOLS_GEOMETRY=OFF`) -- included directly rather than
    vendoring a second copy of the same MIT-licensed library.
  - **Scope bounded and documented in the header**: only the first
    primitive of the first mesh is imported (one skinned rig, not a
    multi-mesh scene); indices must be present; a skin's inverse bind
    matrices must be provided (no silent identity fallback); a joint's
    parent must resolve to another joint in the same skin's own joint list
    (an unskinned skeleton root above it becomes a root, parent `-1`); an
    embedded base-color texture must come via a glTF bufferView (the
    standard `.glb` convention) -- a bare external-file or data-URI
    `image.uri` isn't resolved, since `cgltf_load_buffers()` only decodes
    the top-level "buffers" array, not "images" directly.
  - Unskinned-vertex default fixed during implementation: `ImportedVertex`'s
    weight default was originally all-zero, which would skin every vertex
    to a zero matrix (invisible) for a mesh imported without a
    `WEIGHTS_0` accessor -- changed to full weight on joint 0, so an
    unskinned import still renders correctly bound to `SkinnedMesh`'s
    always-uploaded identity bone 0, without needing a separate code path.

  **`tests/mesh_import_test.cpp` + `tests/fixtures/skinned_wheel_test.gltf`**:
  a hand-authored fixture (generated by a one-off Python script, not a real
  art asset -- no precedent for a committed binary test asset anywhere in
  this repo, so this follows `determinism_test`'s existing "small hand-
  authored fixture read via a repo-relative `WORKING_DIRECTORY`" pattern,
  just in glTF/JSON form) -- a 4-vertex quad ("wheel") fully weighted to a
  child joint of a root joint, plus a flat-color material. Asserts vertex
  positions/uv/indices, joint weights, the parent/name/local-bind-matrix/
  inverse-bind-matrix values for both joints, and that malformed input
  reports failure rather than a partial result.

  **`src/render/vertex_skinned.h`**: `PosNormalUVSkinnedVertex`, this
  port's existing "plain struct + `static layout()`" vertex-format
  convention (`vertex_textured.h`'s `PosNormalUVVertex`, etc.), extended
  with 4x `Attrib::Indices` (`Uint8`, non-normalized) and 4x `Attrib::Weight`
  (`Float`).

  **`src/render/shaders/vs_skinned.sc` + `varying_skinned.def.sc`**: GPU
  linear-blend skinning (weighted sum of up to 4 bone matrices from a
  32-entry `u_boneMatrices` uniform array), reusing `fs_textured_lit.sc`
  unchanged as its fragment stage (no new fragment shader needed -- same
  `v_normal`/`v_texcoord0` outputs, just sourced from skinned geometry).
  Compiles clean through `shaderc` for all three profiles (spirv/glsl/essl).

  **`src/render/texture_import.h`/`.cpp`**: decodes embedded PNG/JPEG bytes
  to a plain RGBA8 buffer. Uses `stb_image.h` directly (already vendored as
  one of `bimg`'s own 3rdparty dependencies) rather than `bimg`'s public
  `imageParse()` API -- the pinned `bimg` version's public header only
  exposes `imageParse()` for container formats (DDS/KTX/PVR3); its actual
  PNG/JPEG decode path is internal to `image_decode.cpp`, not part of the
  public surface in this vendored version. `stb_image.h` is the same decoder
  `bimg` already uses internally for those formats, so this isn't a step
  down in reliability, just a more direct route to it -- and avoids linking
  `bimg_decode` for a public API this pinned version doesn't actually
  expose.

  **`src/render/skinned_mesh.h`/`.cpp`**: the GPU-calling consumer --
  uploads `ImportedMesh` to vertex/index/texture buffers, exposes
  `setBoneMatrices()`/`draw()`. Deliberately not unit-tested (this port's
  established convention: bgfx-calling code is verified by a headless
  screenshot, not a ctest -- no test target anywhere in this repo
  instantiates real bgfx buffers/programs/draws). The shared shader
  program/uniforms are function-local statics (lazily created once,
  guarded by `bgfx::isValid()`) rather than `Renderer` fields, so this class
  stays self-contained until Step 3 actually wires it into `Renderer`'s
  own init()/shutdown() lifecycle.

  **GPU verification (temporary debug hook, removed after use -- same
  convention as this session's earlier `LHT_SUBSTEP_DEBUG`/`LHT_TIRE_DEBUG`
  hooks)**: loaded the test fixture, uploaded it as a `SkinnedMesh`, and
  rendered it twice with different bone-matrix palettes for joint 1
  ("wheel") into its own view, capturing a headless `xvfb-run` screenshot of
  each. Found and fixed two real bugs this surfaced (neither would have been
  caught by ctest alone, since no bgfx-free test touches the GPU path):
  - **Attribute type mismatch**: `varying_skinned.def.sc` originally declared
    `a_indices` as `uvec4` (copying `bgfx`'s own `debugdraw` example
    verbatim), but this project's vertex layout (`vertex_skinned.h`) binds
    `Attrib::Indices` via the plain float attribute path (no `.asInt()`
    marker) -- a real type mismatch between what the vertex array actually
    provides and what the shader declares. Every draw call silently
    rendered nothing under this project's headless GL backend (every bgfx
    resource still reported `isValid()==true`; the mismatch produced no
    visible error at any trace level available here, only an empty frame).
    Fixed by declaring `a_indices` as a plain `vec4` instead -- the shader's
    existing `int(a_indices.x)` cast works identically on a float or a
    uint.
  - **Screenshot capture ordering**: `Renderer::requestScreenshot()` must be
    called *before* the `bgfx::frame()` call that actually flushes the draw
    it's meant to capture, not after -- confirmed by tracing through the
    real per-frame loop's own ordering (`renderFrame()`'s trailing
    `bgfx::frame()`, then `requestScreenshot()`, captured by the *next*
    iteration's `renderFrame()`+`bgfx::frame()`, which works there only
    because `renderFrame()` resubmits the whole scene every frame anyway).
  After both fixes, the two screenshots showed the quad's on-screen bounding
  box shift substantially (from `x∈[564,715]` to `x∈[837,1051]`, at
  1280x720) between the identity and rotate+translate bone-matrix cases --
  direct proof the GPU skinning pipeline (buffers → `vs_skinned.sc` →
  per-vertex bone blending → visible displacement) works end to end, not
  just that it compiles and creates valid resources.

  **Verified**: native `ctest` 28/28 (new `mesh_import_test`). 4-track smoke
  test (`LHT_FORCE_RACE=1 LHT_HEADLESS_FAST=1`, 3000 frames): `wreckCount=0`
  on every track. Natural-start fleet-wide regression (track 0, 20000
  frames): `wreckCount=0`, unchanged from Step 1c's baseline (expected --
  this step adds no sim/physics code at all, only dormant rendering
  infrastructure).

  **Not yet done** (Step 3's job): wiring `SkinnedMesh` into `Renderer`
  itself, replacing the flat car quad; asset staging (`assets/models/*.glb`,
  `POST_BUILD` copy step, `pages.yml` deploy step) for a real artist-authored
