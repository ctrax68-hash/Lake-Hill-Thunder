...
// (S.sound, S.volume, S.tilt, S.tiltG, S.camMode, S.shakeT) are intentionally
// left out, same rationale as Car's progHist/replayHist. `cars`/`player`/
// `order`/`finishOrder` are NOT stored here -- they're managed as an
// explicit std::vector<Car>& passed around instead of embedded in this
// struct, since C++ doesn't need JS's single-global-object convenience and
// explicit parameters make ownership clearer.
struct RaceState {
